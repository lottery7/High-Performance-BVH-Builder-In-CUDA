    ...
    int ignore_face)
{
  ...
  for (unsigned int fi = 0; fi < nfaces; ++fi) {
    ...
    if (intersect_ray_triangle(orig, dir, a, b, c, tMin, tMax, /*backface*/ false, t, u, v)) return true;
  }
  return false;
}
    ...
    unsigned int nfaces)
{
  ...
  for (int fi = 0; fi < nfaces; ++fi) {
    ...
    if (intersect_ray_triangle(ray_origin, ray_direction, v0, v1, v2, tMin, tBest, /*backface*/ false, t, u, v)) {
      tBest = t;
      faceIdBest = fi;
      uBest = u;
      vBest = v;
  ...
  if (faceIdBest >= 0) {
    ...
    for (int s = 0; s < AO_SAMPLES; ++s) {
      ...
      if (any_hit_from(Po, d, vertices, faces, nfaces, faceIdBest)) ++hits;
    }
...
namespace cuda
{
  void ray_tracing_render_brute_force(
      ...
      unsigned int nfaces)
  {
    ray_tracing_render_brute_force<<<gridSize, blockSize, 0, stream>>>(
        vertices,
        faces,
        framebuffer_face_id,
