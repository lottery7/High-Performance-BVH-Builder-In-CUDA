...
namespace
{
  __device__ __forceinline__ float3 load_vertex(const float *vertices, unsigned int vi)
  {
    return {
        __ldg(&vertices[3 * vi + 0]),
        __ldg(&vertices[3 * vi + 1]),
        __ldg(&vertices[3 * vi + 2]),
    };
  }

  __device__ __forceinline__ uint3 load_face(const unsigned int *faces, unsigned int fi)
  {
    return {
        __ldg(&faces[3 * fi + 0]),
        __ldg(&faces[3 * fi + 1]),
  ...
  __device__ __forceinline__ float3 cross_f3(const float3 &a, const float3 &b)
  {
  ...
  __device__ __forceinline__ float length_f3(const float3 &v) { return sqrtf(v.x * v.x + v.y * v.y + v.z * v.z); }
  __device__ __forceinline__ void make_basis(const float3 &n, float3 &t, float3 &b)
  {
  ...
  static __device__ __forceinline__ bool intersect_ray_triangle(
      const float3 ray_o,
      const float3 ray_d,
      const float3 v0,
      const float3 v1,
  ...
  static __device__ __forceinline__ bool intersect_ray_triangle_any(
      const float3 ray_o,
      const float3 ray_d,
      const float3 v0,
      const float3 v1,
      ...
      float &tHitFar)
  {
    ...
    if (fabsf(ray_d.x) < eps) {
      ...
      if (ray_o.x < box.min_x || ray_o.x > box.max_x) return false;
    } else {
    ...
    if (fabsf(ray_d.y) < eps) {
      if (ray_o.y < box.min_y || ray_o.y > box.max_y) return false;
    } else {
    ...
    if (fabsf(ray_d.z) < eps) {
      if (ray_o.z < box.min_z || ray_o.z > box.max_z) return false;
    } else {
