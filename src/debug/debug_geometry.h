...
inline bool save_ply_triangles_ascii(const std::string& filename, const std::vector<point3f>& vertices, const std::vector<point3u>& triangles)
{
  const std::uint32_t vertex_count = static_cast<std::uint32_t>(vertices.size());
  const std::uint32_t face_count = static_cast<std::uint32_t>(triangles.size());
