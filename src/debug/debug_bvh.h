...
namespace debug
{
  namespace bvh_detail
  {
    ...
    inline void write_ply_header(std::ofstream& out, std::uint32_t vertex_count, std::uint32_t face_count)
    {
    ...
    inline void write_box_faces(std::ofstream& out, std::uint32_t base)
    {
      ...
      auto v = [base](std::uint32_t local_index) { return base + local_index; };
  ...
  inline void dump_scene_geometry_ply(const std::string& filename, const SceneGeometry& scene)
  {
    ...
    const std::uint32_t* fdata = reinterpret_cast<const std::uint32_t*>(scene.faces.data());

    std::ofstream out(filename.c_str());
    if (!out.is_open()) return;

    bvh_detail::write_ply_header(out, static_cast<std::uint32_t>(nverts), static_cast<std::uint32_t>(nfaces));
  ...
  inline void dump_bvh_boxes_ply(const std::string& filename, const std::vector<BVHNodeGPU>& nodes, std::uint32_t min_depth, std::uint32_t max_depth)
  {
    ...
    if (vert_count64 > std::numeric_limits<std::uint32_t>::max() || face_count64 > std::numeric_limits<std::uint32_t>::max()) {
      // Clamp just in case; resulting file will be incomplete but still valid.
      vert_count64 = std::min<std::uint64_t>(vert_count64, std::numeric_limits<std::uint32_t>::max());
      face_count64 = std::min<std::uint64_t>(face_count64, std::numeric_limits<std::uint32_t>::max());
    }

    std::ofstream out(filename.c_str());
    if (!out.is_open()) return;

    bvh_detail::write_ply_header(out, static_cast<std::uint32_t>(vert_count64), static_cast<std::uint32_t>(face_count64));
      ...
      std::uint32_t max_depth)
  {
    ...
    const std::uint32_t vertex_count = static_cast<std::uint32_t>(selected_nodes);
