...
static inline int find_split(const std::vector<uint32_t>& codes, int first, int last)
{
  ...
  auto delta = [&](int i, int j) -> int { return common_prefix(codes, N, i, j); };
    ...
    std::vector<uint32_t>& outLeafTriIndices)
{
  ...
  point3f cMin{+std::numeric_limits<float>::infinity(), +std::numeric_limits<float>::infinity(), +std::numeric_limits<float>::infinity()};
  point3f cMax{-std::numeric_limits<float>::infinity(), -std::numeric_limits<float>::infinity(), -std::numeric_limits<float>::infinity()};
  ...
  std::sort(prims.begin(), prims.end(), [](const Prim& a, const Prim& b) { return a.morton < b.morton; });
