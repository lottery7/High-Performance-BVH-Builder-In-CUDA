...
static inline void triangulateFan(const std::vector<Idx>& poly, std::vector<point3u>& out)
{
  ...
  for (size_t i = 1; i + 1 < poly.size(); ++i) {
    out.emplace_back(point3u{static_cast<uint32_t>(poly[0]), static_cast<uint32_t>(poly[i]), static_cast<uint32_t>(poly[i + 1])});
  }
}
...
namespace ply_detail
{
  ...
  static inline T readLE(std::istream& is)
  {
    ...
    std::reverse(reinterpret_cast<unsigned char*>(&v), reinterpret_cast<unsigned char*>(&v) + sizeof(T));
#endif
    return v;
  }
...
SceneGeometry loadPLY(const std::string& path)
{
  ...
  if (isAscii) {
    ...
    for (uint64_t i = 0; i < vertexCount; ++i) {
      ...
      scene.vertices[static_cast<size_t>(i)] = point3f{float(scalars[size_t(xPos)]), float(scalars[size_t(yPos)]), float(scalars[size_t(zPos)])};
    }
