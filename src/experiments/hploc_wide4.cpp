...
{
  std::cout << "\n=== Experiment: H-PLOC-Wide4" << std::endl;
  ...
  WideBVHNode4* d_wide_bvh = nullptr;
  ...
  CUDA_SAFE_CALL(cudaMallocAsync(&d_wide_bvh, sizeof(WideBVHNode4) * max_binary_nodes, stream));
  ...
  for (int iter = 0; iter < BENCHMARK_ITERS + WARMUP_ITERS; ++iter) {
    ...
    timer conversion_t;
    cuda::hploc::convert_to_wide<4>(stream, d_binary_bvh, d_wide_bvh, d_tasks, d_next_task, d_next_wide_node, d_block_counter, n_faces);
    CUDA_SYNC_STREAM(stream);
  ...
  std::cout << "H-PLOC-Wide4 BVH2 build times (in seconds) - " << stats::valuesStatsLine(bvh2_build_times) << std::endl;
  std::cout << "H-PLOC-Wide4 conversion times (in seconds) - " << stats::valuesStatsLine(conversion_times) << std::endl;
  std::cout << "H-PLOC-Wide4 total build times (in seconds) - " << stats::valuesStatsLine(total_build_times) << std::endl;
  std::cout << "H-PLOC-Wide4 total build performance: " << build_mtris << " MTris/s" << std::endl;
  ...
  for (int iter = 0; iter < BENCHMARK_ITERS + WARMUP_ITERS; ++iter) {
    ...
    cuda::rt_hploc_wide<4>(stream, width, height, scene.d_vertices, scene.d_faces, d_wide_bvh, fb.d_face_id, fb.d_ao, scene.d_camera);
    CUDA_SYNC_STREAM(stream);
  ...
  std::cout << "H-PLOC-Wide4 ray tracing frame render times (in seconds) - " << stats::valuesStatsLine(rt_times) << std::endl;
  std::cout << "H-PLOC-Wide4 ray tracing performance: " << mrays << " MRays/s" << std::endl;
  std::cout << "H-PLOC-Wide4 total frame time: " << stats::median(total_build_times) + stats::median(rt_times) << " seconds" << std::endl;

  auto res = RayTracingResult();
  fb.readback(res.face_ids, res.ao);
  save_framebuffers(results_dir, "with_H-PLOC-Wide4", res.face_ids, res.ao);
