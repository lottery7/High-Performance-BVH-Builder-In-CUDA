...
RayTracingResult run_kitten_lbvh(const SceneDevice& scene_gpu, FramebuffersDevice& fb, const std::string& results_dir, int n_iters)
{
  ...
  CUDA_SAFE_CALL(cudaMalloc(&d_lbvh_nodes, (2 * n_faces - 1) * sizeof(BVHNode)));
  ...
  for (int iter = 0; iter < n_iters + WARMUP_ITERS; ++iter) {
    ...
    cuda::convert_to_bvh_nodes(0, lbvh.internalNodes(), lbvh.objects(), lbvh.objectIds(), d_lbvh_nodes, d_sorted_indices, n_faces);
    CUDA_CHECK_STREAM(0);
  ...
  report_sah(0, n_faces, d_lbvh_nodes);
  ...
  CUDA_SAFE_CALL(cudaFree(d_lbvh_nodes));
