...
#pragma region LBVH
	...
	LBVH::aabb LBVH::bounds() {
        ...
        LBVH::node* LBVH::internalNodes() const { return impl->d_nodes.data().get(); }

        LBVH::aabb* LBVH::objects() const { return impl->d_objs.get(); }

        uint32_t* LBVH::objectIds() const { return impl->d_objIDs.data().get(); }
